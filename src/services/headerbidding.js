...
export function initializeBiddingServices({
  ...
  // Waits for all header bidding services to be initialized before telling the service it's ready
  // to retrieve bids.
...
/**
* @desc Fetches a bid for an advertisement based on which services are enabled on unit and the
* wrapper.
* @param {object} obj - An object containing all of the function arguments.
* @param {Object} obj.ad - An object containing the GPT ad slot.
* @param {string} obj.id - A string containing the advertisement id corresponding to the div the
* advertisement will load into.
* @param {string} obj.slotName - A string containing the slot name of the advertisement, for
* instance '1234/adn.com/homepage'.
* @param {Array} obj.dimensions - An array containing all of the applicable sizes the
* advertisement can use.
* @param {Object} obj.wrapper - An object containing all of the wrapper settings.
* @param {Array} obj.bidding - Contains all of the applicable bid data, such as which vendors to
* use and their placement ids.
  ...
  const prebidBids = new Promise((resolve) => {
    if (wrapper.prebid && wrapper.prebid.enabled) {
      ...
      queuePrebidCommand.bind(
        ...
        fetchPrebidBids(
          ad,
          wrapper.prebid.useSlotForAdUnit ? slotName : id, timeout,
          adInfo,
          prerender,
          () => {
